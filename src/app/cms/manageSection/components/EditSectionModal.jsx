...
const CreateSectionModal = ({ setIsShowCreateModal, isShowCreateModal,refresh, data,setSelectedSection }) => {
    const [api, contextHolder] = notification.useNotification();

    const [isLoading,setIsLoading] = useState(false);

    const [isOpenAddItemModal,setIsOpenAddItemModal] = useState(false);
    const [listItem,setListItem] = useState(data?.options||[]);
    ...
    const handleOk = async (values) => {
        try{
            setIsLoading(true);
            const res=await manageSectionApi.editSection([{_id:data?._id,...values,options:listItem}])
            openNotificationWithIcon("success","Thông báo","Sửa section thành công");
            refresh();
            handleCancel()
        }
        catch (e) {
    ...
    return (
        ...
        >
            ...
            <Spin spinning={isLoading}>
                <Formik
                    initialValues={{title:data?.title||"", appearTransition:data?.appearTransition||"",type:data?.type||""}}
                    ...
                    {({ values,handleSubmit,setFieldValue,errors, touched,resetForm }) => {
                        ...
                        return (<Form>
                                <div className={'flex justify-around gap-[15px] mb-[12px]'}>
                                    ...
                                    <SelectInput name={"appearTransition"} defaultValue={values?.appearTransition} placeholder={"Chọn hiệu ứng xuất hiện"}
                                    ...
                                    <SelectInput name={"type"} defaultValue={values?.type} placeholder={"Chọn loại section"}
