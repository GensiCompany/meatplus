...
const CreateMenuModal = ({ setIsShowCreateModal, isShowCreateModal, refresh }) => {
    ...
    const createMenu = async (values) => {
        try {
            if(!content){
                ...
                setIsLoading(true);
                const res = await manageMenuApi.createMenu({...values,content,imageUrl:tempItem?.imageUrl});
