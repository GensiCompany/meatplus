...
const fetchDetailNews = async (id)=>{
    try{
        const res =await PromotionApi.getDetail(id);
...
export async function generateMetadata() {
    ...
    return {
        title: data?.name || "Tin tức khuyến mãi",
        description: data?.shortDescription || "Thông tin khuyến mãi mới nhất.",
        openGraph: {
            title: data?.name || "Tin tức khuyến mãi",
            description: data?.shortDescription || "Thông tin khuyến mãi mới nhất.",
            images: [
...
export default async function CMS() {
    ...
    const otherNews = await PromotionApi.getAll(1,5);
    const otherNewsData=otherNews?.data?.discounts;
        ...
        <div className={'flex flex-col md:flex-row justify-center gap-[10px] sm:gap-0 w-full max-w-[1200px] py-[30px]'}>
            <div className={'px-[10px] md:px-[30px] w-full md:w-3/4'}>
                <div className={'relative'}>
                    <Image src={getImageLink(data?.imageUrl)} alt={"anh tin tuc"} width={1920} height={1080} className={'w-full object-cover'}></Image>
                    <div
                ...
                <div>
                    <a href={"/category/khuyen-mai"} className={"mt-[24px] text-[#334862] text-[12px] font-[700]"}>Khuyến mại</a>, <a href={"/category/tin-tuc"} className={"mt-[24px] text-[#334862] text-[12px] font-[700]"}>Tin
                        tức</a>
                </div>
                <p className={'text-[28px] text-[#555] font-[700]'}>{data?.name}</p>
                <div className={"bg-[rgba(0,0,0,.1)] h-[3px] w-[30px] my-[16px]"}></div>
                <p className={'text-[12px] text-[#777]'}>Đăng vào <span className={'text-black'}>{createdDate?.getDate() + "/" + createdDate.getMonth() + 1 + "/" + createdDate.getFullYear()}</span> bởi <span className={'text-black'}>MeatPlus</span>
                </p>
                ...
                {randomNews?.length > 0 && (
                    <div className={'border-[1px] border-[#c0c0c0] border-r-0 border-l-0 mt-[15px] flex items-center'}>
                        <div className={'px-[10px] py-[25px] border-[1px] border-t-0 border-b-0 border-l-0 border-[#c0c0c0] w-1/2 cursor-pointer'}>
                            <a href={`/category/khuyen-mai/${createUrlFriendly(randomNews[0]?.name + "-" + randomNews[0]["_id"])}`} className={'flex gap-[10px] items-center'}>
                        ...
                        </div>
                        {
                            ...
                            <div className={'px-[10px] py-[25px] w-1/2 cursor-pointer'}>
                                <a href={`/category/khuyen-mai/${createUrlFriendly(randomNews[1]?.name + "-" + randomNews[1]["_id"])}`} className={'flex gap-[10px] items-center'}>
